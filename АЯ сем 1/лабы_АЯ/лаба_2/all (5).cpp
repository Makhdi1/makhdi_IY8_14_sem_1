...
int main() {
    ...
    bool peshka_mega_ataka = positionB.second - positionA.second == 1 && ((positionA.first == 1 && positionB.second - positionA.second == 1) || (positionA.first == 8 && positionB.second - positionA.second == -1) || (positionA.first > 1 && positionA.first < 8 && std::abs(positionB.second - positionA.second) == 1));
    
    bool peshka_hyper_hod_i_mega_ataka = peshka_hyper_hod || peshka_mega_ataka;

    // std::cout<<"Ладья угрожает: "<<ladya<<std::endl;
